...
export class GarmentGLRenderer {
  ...
  private uploadImage(tex: WebGLTexture, source: ImageSource): boolean {
    const gl = this.gl;
  ...
  private drawSource(
    ...
    if (upload) {
      ...
      } else if (!this.uploadImage(tex, source)) {
        return;
      }
    }
    const { w, h } = sourceDimensions(source);
  ...
  private presentCamera(base: { x: number; y: number }, depth: number) {
    return {
      x: clamp(base.x * depth, -(PRESENT_ZOOM - 1), PRESENT_ZOOM - 1),
      y: clamp(base.y * depth, -(PRESENT_ZOOM - 1), PRESENT_ZOOM - 1),
    };
  }
  ...
  renderPhotoBackground(
    ...
    if (this.drawImageLayer(backImage, this.bottomTex, cam, false, false, overscan)) {
      this.bottomEverReady = true;
  ...
  // Mid + scratchable front on a transparent canvas; holes stay in reference frame.
  renderPhotoForeground(
    ...
    if (frontReady || this.fgEverReady) {
      gl.bindFramebuffer(gl.FRAMEBUFFER, null);
      gl.viewport(0, 0, this.width, this.height);
      gl.useProgram(this.composite);
      gl.enable(gl.BLEND);
  ...
  // Linked parallax layers: back/mid/front share one camera; scratch FBO stays in
  // the reference frame so holes remain glued to the garment mesh.
  renderImageLayers(
    backImage: HTMLImageElement | null,
    midImage: HTMLImageElement | null,
    frontImage: HTMLImageElement | null,
  ...
  // scratch holes stay in the reference frame.
  renderImages(
    ...
    const gl = this.gl;
    const bgCamX = clamp(backgroundCamera.x, -(PRESENT_ZOOM - 1), PRESENT_ZOOM - 1);
    const bgCamY = clamp(backgroundCamera.y, -(PRESENT_ZOOM - 1), PRESENT_ZOOM - 1);
    ...
    if (bottomImage && bottomImage.complete && bottomImage.naturalWidth > 0) {
      this.drawSource(this.blit, this.bottomTex, bottomImage, false, bgCamX, bgCamY, PRESENT_ZOOM);
      this.bottomEverReady = true;
    } else if (bottomImage && this.bottomEverReady) {
      this.drawSource(this.blit, this.bottomTex, bottomImage, false, bgCamX, bgCamY, PRESENT_ZOOM, false);
    }

    const fgReady = !!foregroundImage && foregroundImage.complete && foregroundImage.naturalWidth > 0;
    if (!fgReady && !this.fgEverReady) return;
    ...
    if (fgReady) {
      ...
      gl.clearColor(0, 0, 0, 0);
      gl.clear(gl.COLOR_BUFFER_BIT);
      this.drawSource(this.blit, this.fgTex, foregroundImage!, foregroundChroma);
